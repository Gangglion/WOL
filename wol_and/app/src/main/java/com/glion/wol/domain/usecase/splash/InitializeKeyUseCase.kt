...
class InitializeKeyUseCase @Inject constructor(
    ...
    operator fun invoke() : Flow<Unit> = flow {
        ...
        val rsaKeyPair = initializeRepository.loadOrCreateRSAKey()

        // 2. 생성된 RSA 키의 공개키 가져오기
        val rsaPublicKey = rsaKeyPair.public.encoded

        // 3. AES 키 존재 확인
        val isExistAesKey = initializeRepository.isExistAESKey()

        if(isExistAesKey) {
            // 4-1. AES 키가 존재한다면 메모리에 로드
            initializeRepository.loadAESKey()
        } else {
            // 4-2. AES 키가 없다면 API 호출 후 저장
            val encryptedAesKey = initializeRepository.exchangeKey(rsaPublicKey)
            initializeRepository.saveAESKey(encryptedAesKey)
        }
